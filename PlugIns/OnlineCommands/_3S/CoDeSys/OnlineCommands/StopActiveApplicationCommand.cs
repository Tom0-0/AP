...
{
    [TypeGuid("{fd2ae898-05a5-478c-9b31-20c0c112cab1}")]
    ...
    public class StopActiveApplicationCommand : IStandardCommand, ICommand, IHasContextlessName
    {
        private static readonly string[] BATCH_COMMAND = new string[2] { "online", "stopactiveapp" };
        ...
        public string Description => ((IEngine)APEnvironment.Engine).ResourceManager.GetString(typeof(Strings), "StopActiveApplicationCommand_Description");

        public string Name => ((IEngine)APEnvironment.Engine).ResourceManager.GetString(typeof(Strings), "StopActiveApplicationCommand_Name");
        ...
        public bool Enabled
        {
            get
            {
                ...
                {
                    return OnlineCommandHelper.CanStop(activeAppObjectGuid);
                }
                return false;
            }
        }
        ...
        public string ContextlessName => ((IEngine)APEnvironment.Engine).ResourceManager.GetString(typeof(Strings), "StopActiveApplicationCommand_ContextlessName");
        ...
        public void ExecuteBatch(string[] arguments)
        {
            ...
            try
            {
                _bIsExecutionInProgress = true;
                Guid activeAppObjectGuid = OnlineCommandHelper.ActiveAppObjectGuid;
                if (activeAppObjectGuid != Guid.Empty && OnlineCommandHelper.CanStop(activeAppObjectGuid))
                {
                    OnlineCommandHelper.Stop(activeAppObjectGuid);
                }
            }
            ...
            {
                APEnvironment.MessageService.Error(ex2.Message, "ErrorStopActiveApp", Array.Empty<object>());
            }
            finally
            {
                _bIsExecutionInProgress = false;
